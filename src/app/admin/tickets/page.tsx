  ...
  const AddForm=()=>{
    ...
    return <form onSubmit={submit}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem'}}>
        ...
        <FG label="Role"><select style={inp} value={d.role} onChange={e=>setD({...d,role:e.target.value})}><option value="employee">Employee</option><option value="admin">Admin</option></select></FG>
      </div>
              ...
              {employees.length===0
                ...
                <tr key={e._id||e.id} style={{borderBottom:'1px solid var(--border-mid)'}}>
                  ...
                  <td style={{padding:'12px 1.4rem',display:'flex',gap:5,flexWrap:'wrap'}}>
                    ...
                    <button onClick={()=>handleDelete(e._id||e.id)} style={{padding:'3px 8px',borderRadius:5,border:'1px solid rgba(198,40,40,0.25)',background:'rgba(198,40,40,0.08)',color:'#c62828',cursor:'pointer',fontSize:'0.68rem',fontWeight:600}}>🗑️ Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
